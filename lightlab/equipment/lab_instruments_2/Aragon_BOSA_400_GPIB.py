...
from lightlab.util.data.one_dim import SpectrumGHz
# import visa
...
if(len(logger.handlers) == 0): # check if the logger already exists
    # create logger
...
class Aragon_BOSA_400 (VISAInstrumentDriver):
    ...
    def __init__(self, name='BOSA 400 OSA', address=None, timeout=30*1000, **kwargs):
        VISAInstrumentDriver.__init__(self, name=name, address=address, **kwargs)
        self.interface = visa.ResourceManager().open_resource(address, timeout=timeout)
    ...
    def write(self, command=None):
        if command is not None:
            logger.debug("Sending command '" + command + "' using GPIB interface...")
            try:
                self.interface.write(command)
            except Exception as e:
                logger.exception("Could not send data, command %r",command)
                print(e)
                raise e

    def read(self):
        logger.debug("Reading data using GPIB interface...")
        while(1):
            try:
                ...
                logger.exception("Could not read data")
                print(e)
                raise e
        logger.debug("All data readed!")
    ...
    def application(self, app=None):
        if app is not None and app in self.__apps:
            try:
                ...
                self.__currApp = None
                logger.exception("Could not choose the application")
                print(e)
                raise e
    ...
    def ask_TRACE_REAL(self):
        ...
        # print("Number of samples: ", NumPoints)
        self.write("TRAC?")
    ...
    def spectrum(self, form='REAL'):
        ...
        if(form=='ASCII'):
            ...
            logger.exception("Please choose form 'REAL' or 'ASCII'")
        return SpectrumGHz(x, y, inDbm=True)
    ...
    def CAParam(self, avgCount='CONT', sMode='HR', noiseZero=False):
        if self.__currApp == 'CA' and avgCount in self.__avg and sMode in self.__sMode:
            try:
                ...
                logger.exception("Could not set CA parameter")
                print(e)
                raise e
        else:
    ...
    def CAInput(self, meas='IL', pol='1'):
        if meas in self.__CAmeasurement and pol in self.__CAPolarization:
            try:
                ...
                logger.exception("Could not set input parameters to CA")
                print(e)
                raise e
        else:
    ...
    def TLSwavelength(self, waveLength=None):
        if waveLength is not None and self.__currApp == 'TLS':
            try:
                ...
                logger.exception("Could not set the wavelength for TLS")
                print(e)
                raise e
        else:
...
# class Aragon_BOSA_400_PROLOGIX(PrologixGPIBEthernetDevice):
